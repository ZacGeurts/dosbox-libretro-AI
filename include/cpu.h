...
[[gnu::always_inline]] static inline void CPU_HW_Interrupt(Bitu num) {
    CPU_Interrupt(num, 0, reg_eip);
}

[[gnu::always_inline]] static inline void CPU_SW_Interrupt(Bitu num, Bitu oldeip) {
    CPU_Interrupt(num, CPU_INT_SOFTWARE, oldeip);
}

[[gnu::always_inline]] static inline void CPU_SW_Interrupt_NoIOPLCheck(Bitu num, Bitu oldeip) {
...
[[gnu::always_inline, gnu::hot]] static inline void CPU_SetFlagsd(Bitu word) {
    Bitu mask = cpu.cpl ? FMASK_NORMAL : FMASK_ALL;
    CPU_SetFlags(word, mask);
}

[[gnu::always_inline, gnu::hot]] static inline void CPU_SetFlagsw(Bitu word) {
